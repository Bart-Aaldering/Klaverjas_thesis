...
def test_agent(rounds_amount: int, process_num: int, mcts_steps: int, number_of_simulations: int, nn_scaler: float, ucb_c_value: int, model_name: str):
    # random.seed(13)
...
def run_test_multiprocess():
    ...
    if multiprocessing:
        with Pool(processes=n_cores) as pool:
            results = pool.starmap(test_agent, [(rounds_per_sim, i, mcts_steps, number_of_simulations, nn_scaler, ucb_c_value, model_name) for i in range(n_cores)])
